...
<body class="nav-fixed">
    ...
    <script>
                ...
                fetch('http://127.0.0.1:8000/admin/recerva/' + value)
                    ...
                    .then((data) => {
                        ...
                        if (data.length > 0) {
                            ...
                            tr.id = tablebody.getElementsByTagName("tr").length;
                            productsSelected.forEach(item => {
                                ...
                                <td><button type="button" name=${tr.id} class="btn btn-secondary" onclick="handleDeleteReg(event);" value=${name}><i class="bi bi-trash"></i></button></td>
                                `;
        ...
        function calculateSubTotal(e) {
            ...
            if (!existingError) { //se puede interpretar así: if(!existingError == true)
        ...
        function handleDeleteReg(e) {
            //let parent = document.getElementById()
            //const indextr = e.currentTarget.name;
            //let celdas = document.getElementById(indextr)
            //remove(document.querySelector("#" + name));
            const nromesa = e.currentTarget.value;
            const tabbody = document.querySelector("#databody" + nromesa);
            let trid = e.currentTarget.name;
            let tr = e.currentTarget.parentNode.parentNode;
            let subtotal = Number(tr.children[5].innerText);
            let productoID = tr.children[3].children[0].value;

            let total = Number(document.querySelector("#total" + nromesa).innerHTML);
            total = total - subtotal;
            document.querySelector("#total" + nromesa).innerHTML = total;
            document.getElementById("total_pedido" + nromesa).value = total;
            console.log(e.currentTarget.parentNode.parentNode.remove());
            
            let arraytabbody = tabbody.getElementsByTagName("tr");
            let longtbody = arraytabbody.length;
            for(i=0; i<longtbody; i++){
                arraytabbody[i].id = i+1;
            }

            /*productsSelected.filter(function(elemento){
                if(elemento.id == trid){
                    productsSelected[trid-1].remove();
                }
            })*/

            productsSelected.forEach(item => {
                if(item.id == productoID){
                    let index = productsSelected.indexOf(item);
            ...
            productFinalSelected.forEach(item => {
                if(item.id == productoID){
                    let index = productFinalSelected.indexOf(item);
                    productFinalSelected.splice(index, 1);
                }
            })
            return;
        }
